...
import { INDUSTRIES, ROLES, getIndustryById, getIndustryBySlug, getRoleById, getRoleBySlug, getRolesByIndustryId, getRolesByIndustrySlug } from "./taxonomy.js";
const server = new Server({ name: "prk-questions-mcp", version: "2.0.0" }, { capabilities: { tools: {} } });
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
    ...
    if (name === "list_roles") {
        let roles = ROLES;
        if (args?.industry_id) {
            roles = getRolesByIndustryId(args.industry_id);
        }
        else if (args?.industry_slug) {
            roles = getRolesByIndustrySlug(args.industry_slug);
        }
        return {
            content: [{
                    type: "text",
    ...
    if (name === "get_industry") {
        let industry;
        if (args?.id) {
            industry = getIndustryById(args.id);
        }
        else if (args?.slug) {
            industry = getIndustryBySlug(args.slug);
        }
        if (!industry) {
    ...
    if (name === "get_role") {
        let role;
        if (args?.id) {
            role = getRoleById(args.id);
        }
        else if (args?.slug) {
            role = getRoleBySlug(args.slug);
        }
        if (!role) {
